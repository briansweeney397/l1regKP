function[x,X,LG,LStop] = MM_ParamSel_GSVD2(U1,U2,V1,V2,X1,X2,X1i,X2i,UpsF,M,b,method,epsilon,tol,lamtol,maxiter,za)
% Apply Majorization-Minimization to the l2-l1 problem where 
% A = kron(A1,A2) and L = kron(L1,L2) have KP structure. 
% The GSVDs of {A1,L1} and {A2,L2} are used to solve the problem and the 
% parameter lambda is selected at each iteration.
%
% Inputs:
...
for i = 1:maxiter
    ...
    if lamcut == 0
        if sel == 1
            LG(i,1) = gcvIterGSVD2(U1,U2,V1,V2,sm,b,wreg);
            conv = 1;
        elseif sel==2
            vint = V2'*reshape(wreg,p,p)*V1;
